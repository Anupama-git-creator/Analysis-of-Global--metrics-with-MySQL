-- drop database countries_1;
-- create database countries_1;
-- use countries_1;
...
SELECT COUNT(*) FROM `countries of the world`;
 ...
 select Country,`Coastline (coast/area ratio)`,`Literacy (%)`,`Pop. Density (per sq. mi.)`,`Arable (%)`,`Crops (%)`,Service 
...
select Country,Region,Population,`Area (sq. mi.)`,`Pop. Density (per sq. mi.)`,`Coastline (coast/area ratio)`,
`Net migration`,`Infant mortality (per 1000 births)` from `countries of the world`;
...
select Country,GDP_per_Capita from `countries of the world`;
...
SET Continent = CASE
    ...
    WHEN Region LIKE '%America%' THEN 'North America'
...
FROM
    `countries of the world`
WHERE
    `Net migration`IS NOT NULL
ORDER BY `Net migration`
LIMIT 1;
-- Micronesia, Fed. St. 	-20.99

-- 15. Find the continent with the most and least net migrants.
SELECT 
